...
bronx1<-read.csv(file.choose(),header=TRUE) 
...
SALE.PRICE<-as.numeric(gsub(",","", bronx1$SALE.PRICE)) 
...
# Model 3
m3<-lm(bronx1$SALE.PRICE~0+log(bronx1$GROSS.SQUARE.FEET)+log(bronx1$LAND.SQUARE.FEET)+factor(bronx1$NEIGHBORHOOD)+factor(bronx1$BUILDING.CLASS.CATEGORY))
summary(m3)
plot(resid(m3))

# Model 4
m4<-lm(bronx1$SALE.PRICE~0+log(bronx1$GROSS.SQUARE.FEET)+log(bronx1$LAND.SQUARE.FEET)+factor(bronx1$NEIGHBORHOOD)*factor(bronx1$BUILDING.CLASS.CATEGORY))
summary(m4)
plot(resid(m4))
#
